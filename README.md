...
--------------------------
игровое поле размером 10 на 10 клеточек.
...
-------------------------
использовать классический JavaScript, без дополнительных библиотек, не использовать canvas.
Каждой клетке на поле должны быть присвоены две координаты: x (по горизонтали) и y (по вертикали). 
...
Яблоко появляется в случайном месте поля, яблоко может появиться только на не занятой змейкой клетке.
...
ПРАВИЛА ИГРЫ
------------
В начале игры змейка располагается в середине поля. Её начальная длина — две клетки.
...
*Если змейка «врезается в стену», она поворачивает и идёт вдоль стены.
